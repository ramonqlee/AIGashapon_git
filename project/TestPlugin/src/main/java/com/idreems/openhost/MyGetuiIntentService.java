...
//        Log.d(TAG, "settag result sn = " + sn + ", code = " + code + ", text = " + getResources().getString(text));
    ...
    private void sendMessage(Context context, String data, int what) {
        Log.d(TAG, MyGetuiIntentService.class.getSimpleName() + " payload : " + data);
        // 广播发送给插件
        if (null == context) {
            return;
        }
        Intent intent = new Intent("com.idreems.openhost.action");
        intent.putExtra(PushConsts.CMD_ACTION,PushConsts.GET_MSG_DATA);
        intent.putExtra("payload", data.getBytes());
